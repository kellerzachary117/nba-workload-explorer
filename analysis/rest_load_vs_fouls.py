"""
Same rest/workload methodology as multiseason_rest_load_vs_ts.py and
rest_load_vs_turnovers.py, applied to personal fouls instead.

Uses fouls per 36 minutes rather than raw fouls per game, for the same reason
TS% and TOV% used rate stats instead of raw counts: raw foul counts mostly
track playing time itself (more minutes on court = more chances to foul),
which is the same confound already corrected for elsewhere. Per-36 fouls is
then z-scored against each player's own season baseline.
"""
import glob
import math
...
MIN_GAMES = 40
MIN_MINUTES = 10.0      # exclude garbage-time cameos from the per-36 rate calc
...
def analyze_season(season):
    ...
    records = []  # pid, name, date, rest_days, rolling7min, fouls_per36
    qualifying = 0
    for pid, games in players.items():
        ...
        for i, (date_i, minutes_i, r) in enumerate(parsed):
            ...
            fouls = float(r["foulsPersonal"])
            fouls_per36 = fouls / minutes_i * 36

            records.append([pid, r["personName"], r["game_date"][:10], rest_days, rolling7, fouls_per36])
    ...
    z_fouls = [r[6] for r in records]

    r_rest = pearson(rest, z_fouls)
    r_load = pearson(load, z_fouls)

    mx1, mx2, my = sum(rest) / n, sum(load) / n, sum(z_fouls) / n
    Sxx1 = sum((x - mx1) ** 2 for x in rest)
    Sxx2 = sum((x - mx2) ** 2 for x in load)
    Sx1x2 = sum((x1 - mx1) * (x2 - mx2) for x1, x2 in zip(rest, load))
    Sx1y = sum((x1 - mx1) * (y - my) for x1, y in zip(rest, z_fouls))
    Sx2y = sum((x2 - mx2) * (y - my) for x2, y in zip(load, z_fouls))
    Syy = sum((y - my) ** 2 for y in z_fouls)
